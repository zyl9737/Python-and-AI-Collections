...
class AdvCrossModalSimple(BaseModel): #模型结构
    ...
    def train(self, sess):
        ...
        adv_loss = []
        emb_loss = []
        for epoch in range(self.model_params.epoch): #开始训练
            if epoch % 1 ==0:
                adv = 0
                emb = 0 
                ...
                for feats, vecs, labels, i in self.data_iter.train_data(): #训练
                ...
                for feats, vecs, labels, i in self.data_iter.test_data(): #测试          
                ...
                k = self.model_params.top_k #topk数目
                avg_precs = []
                #计算文本检索的topk
                for i in range(len(test_txt_vecs_trans)): #按照概率进行排序
            ...
            if epoch == (self.model_params.epoch-1): #保存模型
    ...
    def eval(self, sess): #评价训练后的性能
        ...
        all_k = [50]
        for k in all_k:        
            for i in range(len(test_img_feats_trans)):#计算图像检索的topk
                query_img_feat = test_img_feats_trans[i]
                ground_truth_label = test_labels[i]
