...
class NGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        """
        #一共有三种baseline模型可选，分别是ngcf，gcn和gcmc，具体的paper在上面的注释中。
    ...
    def _init_weights(self):
        ...
        initializer = tf.contrib.layers.xavier_initializer()#初始化权重使用Xavier
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        ...
        maxi = tf.log(tf.nn.sigmoid(pos_scores - neg_scores))#使两者差距尽可能的大
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    """
    plain_adj, norm_adj, mean_adj = data_generator.get_adj_mat()#生成拉普拉斯矩阵
    ...
    if args.pretrain == -1:
        pretrain_data = load_pretrained_data()#预训练数据
    else:
        pretrain_data = None

    model = NGCF(data_config=config, pretrain_data=pretrain_data)#实例化模型
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        t1 = time()
        loss, mf_loss, emb_loss, reg_loss = 0., 0., 0., 0. #初始化
        n_batch = data_generator.n_train // args.batch_size + 1 #计算batch数目

        for idx in range(n_batch):
            users, pos_items, neg_items = data_generator.sample() #采样得到正例负例三元组
            #运行model得到loss
