...
def main(args):
    ...
    if os.path.exists(args.weights):
        ...
        load_weights_dict = {k: v for k, v in weights_dict.items()
                             if model.state_dict()[k].numel() == v.numel()} # 如果两个字典中的同一个键对应的值的元素数量相同
    ...
    for epoch in range(args.epochs):
        ...
        conv1_weights = model.conv1.weight.detach()
        tb_writer.add_histogram(tag="conv1",
                                values=conv1_weights,
                                global_step=epoch)
        layer1_block0_conv1_weights = model.layer1[0].conv1.weight.detach()
        tb_writer.add_histogram(tag="layer1/block0/conv1",
                                values=layer1_block0_conv1_weights,
                                global_step=epoch)
...
if __name__ == '__main__':
    ...
    img_root = "/data/zyl/flower_data/flower_photos"
    
    parser.add_argument('--data-path', type=str, default=img_root)

    # resnet34 官方权重下载地址
    # https://download.pytorch.org/models/resnet34-333f7ec4.pth
    parser.add_argument('--weights', type=str, default='/data/zyl/resnet34-333f7ec4.pth',
